...
@Singleton
class MessageRepository @Inject()(dbConfigProvider: DatabaseConfigProvider)(
  ...
  // peopleテーブル用のTable。MessageTable内から利用するため
  ...
  private class MessagesTable(tag: Tag)
    ...
    def * =
      (id, personId, message, created) <> ((Message.apply _).tupled, Message.unapply)
  }
  ...
  def listMsg: Future[Seq[Message]] = {
    db.run(
      messages.sortBy(_.created.desc).result // createdでソート
    )
  }

  // Person付Message（MessageWithPerson）の取得
  def listMsgWithP(): Future[Seq[MessageWithPerson]] = {
    val query = messages
      .sortBy(_.id.desc)
      .join(people)
      .on(_.personId === _.id) // sortByは機能しない
    db.run(query.result).map { obj =>
      obj
        ...
        .toSeq
        .sortBy(_.message.created.getTime())
        .reverse // Seqをソート
    }
  }
  ...
  def createMsg(personId: Int, message: String): Future[Int] =
    db.run(
      messages +=
        Message(0, personId, message, new Timestamp(new Date().getTime))
    )
}
